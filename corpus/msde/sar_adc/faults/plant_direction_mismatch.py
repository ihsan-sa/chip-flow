...
for `cosim`, "control word polarity inverted", one stage earlier) - here
the comparator output declared digital-to-analog on one side instead of
analog-to-digital."""
...
def plant(ws: Path) -> None:
    ...
    for sig in data.get("interface") or []:
        if sig.get("name") == "cmp_out":
            sig["direction"] = "d2a"
            break
    else:
        raise ValueError(f"{path}: no 'cmp_out' entry to fault")

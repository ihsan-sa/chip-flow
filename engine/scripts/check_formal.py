...
structural wrapper module (never a `bind`: yosys's `read -formal` frontend
silently drops a `bind` statement, so a property attached that way is never
actually checked - proved empirically, not assumed, and the reason this
gate never trusts an ASSERT/COVER id it did not see echoed back in sby's own
model) that instantiates the DUT and adds `` `ifdef FORMAL `` immediate
assert/cover statements (yosys's own formal frontend has no SVA `assert
property (@(...) ...)` support either - also proved empirically; only
procedural, i.e. "immediate", `assert (...)`/`cover (...)` inside an
`always` block work) referencing the DUT's ports directly. spec.yaml names
which wrapper module to prep (`formal: {top, depth}`) and which
...
sby's own model (the id sby echoes back, not a text search - a rename or a
`bind` that silently failed to attach both leave the id missing exactly the
same way); any sby task fails to reach a DONE line at all (a crashed
...
def write_sby(path: Path, sv_files: list[Path], rtl_files: list[Path],
             formal_top: str, mode: str, engine: str, depth: int) -> None:
    files = "\n".join(str(f.resolve()) for f in (*sv_files, *rtl_files))
    reads = "\n".join(f"read -formal -D FORMAL {f.resolve()}"
                      for f in (*sv_files, *rtl_files))
    path.write_text(f"""\
[options]
mode {mode}
depth {depth}
...
def run(argv=None):
    ...
    covers = {pid: c for pid, c in smt_cases.items() if c["type"] == "COVER"}

...
Analog pins that interface.yaml does not name are its supplies; the one
...
def analog_macro(analog: Path, signals: dict[str, str], out_dir: Path) -> dict:
    """Write the analog cell's GDS, LEF, stub and .subckt under out_dir;
    return its ttlib `macros` entry (without location)."""
...
def run(argv=None):
    ...
    if too_big:
        violations = [checklib.violation(
            ...
            f"({ttlib.tile_die_area()})", SCRIPT) for m in too_big]

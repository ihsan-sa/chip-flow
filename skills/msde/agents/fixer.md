...
| top_harden, top_drc, top_lvs | nothing - `top/` is generated by `top_harden`, and every fix lives in a side | `top/`, either side's files |
...
layout, a DRC error inside the macro, a name that differs between the
digital ports and the analog `.subckt`) is NOT yours: name the side, the
file and the finding in OPEN. The orchestrator runs that side's

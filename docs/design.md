...
Sessions run under bwrap with no docker and no nested namespaces, so the image cannot be entered. `eda <tool> [args]` runs one of its binaries in place: it resolves `IMG` from `CHIP_FLOW_IMAGE`, sets the environment the image's own `/etc/profile.d/iic-osic-tools-setup.sh` would (`TOOLS`, `PDK_ROOT`, `PDK=gf180mcuD`, `PDKPATH`, `STD_CELL_LIBRARY`, `SPICE_USERINIT_DIR`, `KLAYOUT_PATH`, `CAD_ROOT` for magic and netgen), and execs the target through `$IMG/lib64/ld-linux-x86-64.so.2 --library-path <image lib dirs>`. `eda python` is `$IMG/usr/bin/python3.12` the same way, with `PYTHONHOME=$IMG/usr`, `PYTHONPATH` set to the image's dist-packages plus the yosys, klayout and vacask python dirs, and user site disabled. Entry points with an `#!/usr/bin/python3` shebang (`librelane`, `cocotb-config`, the tt precheck) run as `eda python -m <module>`, never through the shebang, because the host's `/usr/bin/python3` is a different interpreter.

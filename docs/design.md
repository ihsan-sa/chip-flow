...
Sessions run under bwrap with no docker and no nested namespaces, so the image cannot be entered. `eda <tool> [args]` runs one of its binaries in place: it resolves `IMG` from `EDA_TOOLCHAIN`, sets the environment the image's own `/etc/profile.d/iic-osic-tools-setup.sh` would (`TOOLS`, `PDK_ROOT`, `PDK=gf180mcuD`, `PDKPATH`, `STD_CELL_LIBRARY`, `SPICE_USERINIT_DIR`, `KLAYOUT_PATH`, `CAD_ROOT` for magic and netgen), and execs the target through `$IMG/lib64/ld-linux-x86-64.so.2 --library-path <image lib dirs>`. `eda python` is `$IMG/usr/bin/python3.12` the same way, with `PYTHONHOME=$IMG/usr`, `PYTHONPATH` set to the image's dist-packages plus the yosys, klayout and vacask python dirs, and user site disabled. Entry points with an `#!/usr/bin/python3` shebang (`librelane`, `cocotb-config`, the tt precheck) run as `eda python -m <module>`, never through the shebang, because the host's `/usr/bin/python3` is a different interpreter. The header of `bin/eda` is the authority on how children are shimmed; it caches the shim directory per toolchain and launcher under `~/.cache/chip-flow/shims`.

Tools spawn tools: LibreLane starts openroad, yosys, magic, klayout and netgen; sby starts yosys, yices and yosys-smtbmc; cocotb starts iverilog and vvp; verilator starts the host's make and g++. Children found by name go through a shim directory `eda` generates once per image hash under `~/.cache/chip-flow/shims/` and puts first on `PATH`: one executable per name in `$IMG/foss/tools/bin`, `$IMG/usr/local/bin` and `python3`, each execing `eda <name> "$@"`. Children started by absolute path (yosys finds yosys-abc beside itself) run under the host's loader, so the exported `LD_LIBRARY_PATH` carries the image's tool and system libraries but never its glibc directory, and the host's newer glibc serves them. `eda check-env` prints `check_env.py`'s JSON: every tool's path and version, the PDK version from ciel, and the smoke results M0 lists.
...
Edit classes: `spec_edit` marks everything including holdout, which the testbench writer must rewrite, hold 2. `rtl_edit` marks lint, sim, holdout, mutate, formal, cover, synth, harden, timing, drc, lvs, glsim, precheck, release, hold 1. `tb_edit` marks sim, mutate, cover; `formal_edit` marks formal, mutate; hold 1. `harden_config_edit` (config.json, SDC, info.yaml) marks harden onward, hold 2. `netlist_edit` marks every analog gate; `sizing_edit` marks sim_tt, sim_pvt, bench_strength, mc, lvs, pex_sim, release; `layout_code_edit` marks drc, lvs, pex_sim, release; `interface_edit` marks the top gates and both nested runs' spec_edit, hold 2.
